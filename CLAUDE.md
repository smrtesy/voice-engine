...
**⚠️ הקונטיינר של קלוד הוא Node-only — ה-toolchain של Python לא זמין בו.**
אומת 5/8/2026: לקונטיינר-הראנר יש `node`/`npm` בלבד; `python`, `poetry`,
`ruff`, `pytest`, `mypy` — **כולם חסרים** (`command not found`). לכן **אי-אפשר
להריץ את שער ה-pre-push הזה מקומית בסשן קלוד** — כל ארבע הפקודות ייכשלו על
היעדר-כלי, לא על הקוד. **השער מתבצע ב-CI** (‏`.github/workflows/ci.yml`, שרץ
על כל push ל-`main` וכל PR ומתקין Python+poetry בעצמו). אלה הפקודות שה-CI
מריץ — הן הרפרנס, לא פקודות שקלוד מריץ בעצמו:
...
**מה סשן קלוד כן יכול לעשות מקומית לפני דחיפה** (בלי Python): לקרוא את הדיף
ולוודא שהוא הגיוני, לבדוק תקינות-סינטקס בעין/בכלי-Node אם יש, ולוודא שאף קובץ
לא-קשור לא נגרר. **הסתמכות על CI היא לגיטימית כאן** — אבל בגלל זה חובה לצפות
בריצת ה-CI אחרי הדחיפה (‏`gh run watch` / דף ה-Actions) ולא להכריז "גמור" לפני
שהיא ירוקה; דחיפה ל-`main` = פריסה, וכשל-CI מתגלה רק שם.
