...
## אוטונומיה — הפיך לבד, בלתי-הפיך עוצר

עקרון עומד, זהה לזה של mrtesy-app (שער-האוטונומיה שלו): **פעולה הפיכה — בצע
לבד, בלי לשאול; פעולה בלתי-הפיכה — עצור לאישור אנושי.** לבקש אישור לפעולה
*הפיכה* הוא בדיוק החיכוך שהמערכת באה להסיר — אל תעשה את זה.

- **הפיך → לבד:** push נקי ל-`main` (סעיף כללי דחיפה ומיזוג), כתיבת/עדכון
  שורות ב-DB במהלך עבודה רגילה, פעולות קריאה.
- **בלתי-הפיך → עצור:** כל קריאת API **בתשלום** (Anthropic/Resemble — כלל
  אישור-העלות, גובר תמיד), ומחיקה או דריסה הרסנית של נתוני production קיימים.
- ספק אם משהו הפיך? התייחס אליו כבלתי-הפיך ושאל.
