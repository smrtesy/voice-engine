...
- **מונחים עבריים טהורים — לא ז'רגון לועזי מתועתק** (החלטת המשתמש, 8/2026;
  ...
  (לא "ספייק") · **בדיקת השוואה** (לא "בייק-אוף") · **הרצה** (לא "batch") ·
...
- **אישור עלות לפני כל פעולה שמפעילה API בתשלום.** ה-preprocessor קורא
  ל-Anthropic וה-adapters ל-Resemble/fal — הרצה ידנית / באץ' שמחייבת את
  המשתמש דורשת: מה ירוץ, הערכת עלות (לפריט + סה"כ), והמתנה ל"כן" מפורש על
  העלות. עבודת הסוכן עצמה רצה על המנוי — לא דורשת אישור.
  **שני דיוקים (החלטת המשתמש, 31/07/2026, מסונכרן בשלושת הריפו):**
  (1) הוצאה שהמשתמש מפעיל בעצמו ב-UI של המוצר, כשהמחיר גלוי בנקודת
  הפעולה — הקליק הוא האישור; אישור-צ'אט נדרש רק להוצאה שקלוד יוזם.
  (2) מעטפת מיקרו-בדיקות: בדיקות אימות טכניות קטנות שקלוד יוזם (בדיקת
  שיבוט, probe של מודל) מאושרות מראש **עד $3 מצטבר לבדיקה**, עם דיווח
  העלות בפועל בסיכום; מעבר לזה — אישור רגיל.
...
שורת-הביטול למטה). **סנכרון חובה:** שינוי במדיניות הזו באחד הריפו משוכפל
לשניים האחרים באותו מאמץ, כדי ששלושתם יישארו זהים במבנה (כל ריפו מתאים רק
את החלקים הספציפיים-לפלטפורמה — פקודת ה-build/בדיקות ושיטת אימות-הפריסה).
